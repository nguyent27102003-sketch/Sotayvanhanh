...
shopee_md = "sources/shopee_handbook.md"
tiktok_basic_md = "sources/tiktok_basic_handbook.md"
tiktok_advanced_md = "sources/tiktok_advanced_handbook.md"
...
def get_shopee_products_list():
    ...
    if not os.path.exists(excel_path):
        print(f"Warning: Shopee Excel not found, using fallback.")
        return []
    try:
        import openpyxl
...
def get_tiktok_products_list():
    ...
    if not os.path.exists(excel_path):
        print(f"Warning: TikTok Excel not found, using fallback.")
        return []
    try:
        import openpyxl
        wb = openpyxl.load_workbook(excel_path, data_only=True)
...
<head>
    ...
    <style>
        ...
        * {
        ...
        img.handbook-img {
            max-width: 100%;
            max-height: 55vh; /* Prevents vertical overflow during reading */
            width: auto;
            height: auto;
            object-fit: contain;
            border-radius: 8px;
...
template_foot = f"""
    ...
    <div id="lightbox" class="lightbox-modal" onclick="closeLightbox()">
        ...
        <img class="lightbox-content" id="lightbox-img" alt="Hình ảnh phóng to">
    ...
    <script>
        ...
        // ----------------------------------------------------
        // 9. Lightbox zoomable images (simple display on click)
        // ----------------------------------------------------
        ...
        function setupLightboxListeners() {{
            document.querySelectorAll('.content-body img').forEach(img => {{
                if (img.classList.contains('handbook-img')) {{
                    img.onclick = function() {{
                        openLightbox(this.src, this.alt || "Hướng dẫn minh họa vận hành");
                    }}
                }}
            }});
        }}
